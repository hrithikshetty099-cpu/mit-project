...
import { assignComplaintDepartment, assignComplaintOfficer, assignOfficerDepartment, getAdminComplaints, getAdminDepartments, getAdminOfficers, getAdminUsers, reviewOfficer } from '../api/complaintsApi.js';
...
export default function AdminDashboard() {
  ...
  const load = async () => {
    try {
      const responses = await Promise.all([getAdminUsers(token), getAdminOfficers(token), getAdminDepartments(token), getAdminComplaints(token)]);
      setData({ users: responses[0].data, officers: responses[1].data, departments: responses[2].data, complaints: responses[3].data });
  ...
  const officersForDepartment = (departmentId) => data.officers.filter((officer) => String(officer.department) === String(departmentId) && officer.verification_status === 'approved');
  ...
  return <div className="dashboard-shell"><DashboardSidebar role="admin" /><main className="dashboard-page">
    ...
    <section className="table-panel"><div className="table-heading"><h2>{t('officerAssignments')}</h2><span>{t('department')}</span></div>{data.officers.map((officer) => <div className="admin-row" key={officer.id}><span><b>{officer.name} · {officer.job_id}</b><small>{officer.phone} · {officer.verification_status}</small></span><div><button className="small-action" disabled={officer.verification_status === 'approved'} onClick={async () => { await reviewOfficer(token, officer.id, 'approved'); load(); }}>{t('approve')}</button><button className="small-action danger-action" disabled={officer.verification_status === 'rejected'} onClick={async () => { await reviewOfficer(token, officer.id, 'rejected'); load(); }}>{t('reject')}</button><select value={officer.department} onChange={async (event) => { await assignOfficerDepartment(token, officer.id, event.target.value); load(); }}>{departmentOptions}</select></div></div>)}</section>
    <section className="table-panel"><div className="table-heading"><h2>{t('complaintRouting')}</h2><span>{t('complaints')}</span></div>{data.complaints.map((complaint) => <div className="admin-row" key={complaint.id}><span><b>#{complaint.id} · {complaint.title || complaint.category}</b><small>{complaint.status} · {complaint.department_name || 'Unassigned'}</small></span><div><select value={complaint.department_id || ''} onChange={async (event) => { await assignComplaintDepartment(token, complaint.id, event.target.value); load(); }}><option value="">Unassigned</option>{departmentOptions}</select><select value={complaint.assigned_officer_id || ''} onChange={async (event) => { if (event.target.value) { await assignComplaintOfficer(token, complaint.id, event.target.value); load(); } }}><option value="">Assign officer</option>{officersForDepartment(complaint.department_id).map((officer) => <option key={officer.id} value={officer.id}>{officer.name}</option>)}</select></div></div>)}</section>
  </main></div>;
}
