...
export default function Header() {
  ...
  return <header className="site-header"><Link to="/citizen" className="gov-brand"><span className="emblem" aria-hidden="true">◈</span><span><b>{t('government')}</b><small>{t('appName')}</small></span></Link><nav className="site-nav"><NavLink to="/citizen">{t('reportIssue')}</NavLink><NavLink to="/track">{t('track')}</NavLink><NavLink to="/map">{t('heatmap')}</NavLink><Link to={citizenPath}>{t('citizenLogin')}</Link><Link className="officer-link" to="/officer-login">{t('officerLogin')}</Link><label className="language-picker">{t('language')}<select value={language} onChange={(event) => setLanguage(event.target.value)} aria-label={t('language')}><option value="en">English</option><option value="kn">ಕನ್ನಡ</option><option value="hi">हिन्दी</option></select></label></nav></header>;
}