...
export async function authOfficer(req, res, next) {
  ...
  try {
    ...
    const result = await query("SELECT id, name, job_id, phone, department, verification_status FROM officers WHERE id = $1 AND verification_status = 'approved'", [payload.officerId]);
