...
export async function authOfficer(req, res, next) {
  ...
  try {
    ...
    const result = await query("SELECT id, name, email, phone, municipality_ward, department, verified, verification_status FROM officers WHERE id = $1 AND verified = true AND verification_status = 'approved'", [payload.officerId]);
